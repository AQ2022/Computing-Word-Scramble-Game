...
#SCRAMBLE SOFTWARE WORDS AND CORRECT SOFTWARE WORDS
SoftwareScrambleWords = ['TINGRATOPE STEMSY', 'DINGOC', 'WARESOFT GINTEST' ,'PAP' , 'VIANTIURS', 'GORITHALM' ,'SIGNDE','BASEDATA','FICIALATR TELLIGENCEIN' ,'BUGDEGING','VERSER']
SoftwareCorrectWords  =  ['OPERATING SYSTEM',  'CODING', 'SOFTWARE TESTING' ,'APP' , 'ANTIVIRUS', 'ALGORITHM' ,'DESIGN','DATABASE','ARTIFICIAL INTELLIGENCE','DEBUGGING','SERVER'] 

ran_num = randrange(0, (len(SoftwareScrambleWords)))
scramble_rand_word = SoftwareScrambleWords[ran_num]
points = 0
...
#CODE FOR ALLOWING NEXT BUTTON TO CHANGE WHEN CLICKED!
    def next():
        global ran_num
        ran_num = randrange(0, (len(SoftwareScrambleWords)))
        word.configure(text=SoftwareScrambleWords[ran_num])
...
#CODE FOR YOUR RESULT MESSAGEBOX!
    def result():
        ...
        if user_word == SoftwareCorrectWords[ran_num]:
            points += 10
...
#CODE TO REVEAL CORRECT ANSWER, IF YOU HAVE THE POINTS!
    def reveal_answer():
        ...
        if points > 9:
            points -= 10
            score.configure(text="Score Points ➠  " + str(points))  
            answertext.configure(text=SoftwareCorrectWords[ran_num])
        else:
...
#CREATING WINDOW FOR HARDWARE (SIZE, IMAGE, LABEL, BUTTON)!
    ...
    window.title("Easy Mode: Software Word Scramble! ")
...
#LABELS & BUTTONS
    ...
    title= Label(text='Software',pady=10,bg="#D3D3D3",fg="#3cb371",font="Titillium  25 bold")
    title.pack(anchor="n")
    title.place(x=280, y=0)

    scorereminder= Label(text='Can you guess the Software scrambled words? \n Remember that you can use your score points to find out the answers!',
