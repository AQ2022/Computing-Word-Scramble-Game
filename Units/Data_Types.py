...
#SCRAMBLE DATATYPES WORDS AND CORRECT SOFTWARE WORDS
DataTypesScrambleWords = ['INGSTR', 'BOOEANL' , 'TEGERIN', 'RACCHATER' ,'ATED', 'OATINGFL ONITP' ,'NOLG', 'ROTSH', ' ODIV', 'BLUEOD' ]
DataTypesCorrectWords =  ['STRING' , 'BOOLEAN', 'INTEGER', 'CHARACTER', 'DATE', 'FLOATING POINT', 'LONG', 'SHORT','VOID', 'DOUBLE']

ran_num = randrange(0, (len(DataTypesScrambleWords)))
scramble_rand_word = DataTypesScrambleWords[ran_num]
points = 0
...
#CODE FOR ALLOWING NEXT BUTTON TO CHANGE WHEN CLICKED!
    def next():
        global ran_num
        ran_num = randrange(0, (len(DataTypesScrambleWords)))
        word.configure(text=DataTypesScrambleWords[ran_num])
...
#CODE FOR YOUR RESULT MESSAGEBOX!
    def result():
        ...
        if user_word == DataTypesCorrectWords[ran_num]:
            points += 10
...
#CODE TO REVEAL CORRECT ANSWER, IF YOU HAVE THE POINTS!
    def reveal_answer():
        ...
        if points > 9:
            points -= 10
            score.configure(text="Score Points ➠  " + str(points))  
            answertext.configure(text=DataTypesCorrectWords[ran_num])
        else:
...
#CREATING WINDOW FOR HARDWARE (SIZE, IMAGE, LABEL, BUTTON)!
    ...
    window.title("Medium Mode: Data Types Word Scramble! ")
...
#LABELS & BUTTONS
    ...
    title= Label(text='Data Types',pady=10,bg="#D3D3D3",fg="#ffcc99",font="Titillium  25 bold")
    title.pack(anchor="n")
    title.place(x=255, y=0)

    scorereminder= Label(text='Can you guess the Data Types scrambled words? \n Remember that you can use your score points to find out the answers!',
