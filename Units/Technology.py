...
#SCRAMBLE TECHNOLOGY WORDS AND CORRECT SOFTWARE WORDS
TechnologyScrambleWords = ['REATINGOP SSTEMY', 'INGCOD', 'CIALSO DIAEM', 'TICSROOB', 'NETERINT', 'WARESOFT', 'COLDU PUTCOMING', 'BERCYCURITYSE',
                           'BEW CATLIAPPION', 'CHAMINE RINNGAEL', 'IARTICIALF TELLIGENCEIN', 'COMTATUIPONAL SICCEEN', 'LEXPOCM STEMSY', 'LESSREWI', 'PUTERCOM IENCESC']                                                                                                              
TechnologyCorrectWords =  ['OPERATING SYSTEM', 'CODING', 'SOCIAL MEDIA', 'ROBOTICS', 'INTERNET', 'SOFTWARE', 'CLOUD COMPUTING', 'CYBERSECURITY',
                           'WEB APPLICATION', 'MACHINE LEARNING', 'ARTIFICIAL INTELLIGENCE', 'COMPUTATIONAL SCIENCE', 'COMPLEX SYSTEM', 'WIRELESS', 'COMPUTER SCIENCE']   

ran_num = randrange(0, (len(TechnologyScrambleWords)))
scramble_rand_word = TechnologyScrambleWords[ran_num]
points = 0#IMPORT START GAME HOMEPAGE TO BACK BUTTON! 
def main():
    def back():
        window.destroy()
        import Game
...
#CODE FOR ALLOWING NEXT BUTTON TO CHANGE WHEN CLICKED!
    def next():
        global ran_num
        ran_num = randrange(0, (len(TechnologyScrambleWords)))
        word.configure(text=TechnologyScrambleWords[ran_num])
...
#CODE FOR YOUR RESULT MESSAGEBOX!
    def result():
        ...
        if user_word == TechnologyCorrectWords[ran_num]:
            points += 10
...
#CODE TO REVEAL CORRECT ANSWER, IF YOU HAVE THE POINTS!
    def reveal_answer():
        ...
        if points > 9:
            points -= 10
            score.configure(text="Score Points ➠  " + str(points))  
            answertext.configure(text=TechnologyCorrectWords[ran_num])
        else:
...
#CREATING WINDOW FOR HARDWARE (SIZE, IMAGE, LABEL, BUTTON)!
    ...
    window.title("Hard Mode: Technology Word Scramble! ")
...
#LABELS & BUTTONS
    ...
    title= Label(text='Technology',pady=10,bg="#D3D3D3",fg="#FF7276",font="Titillium  25 bold")
    title.pack(anchor="n")
    title.place(x=250, y=0)

    scorereminder= Label(text='Can you guess the Technology scrambled words? \n Remember that you can use your score points to find out the answers!',
