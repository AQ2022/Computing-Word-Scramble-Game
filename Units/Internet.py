...
#SCRAMBLE INTERNET WORDS AND CORRECT INTERNET WORDS
InternetScrambleWords = ['LAMEI','PTHT', 'RETRUO', 'NPV', 'IW-IF', 'BRROSWE' ,'PI DRESSAD', 'LINKPERHY', ' HOSTTOP' , 'HCARSE GINEEN', 'LUR' , 'ROLDW WEID BEW']
InternetCorrectWords = ['EMAIL','HTTP','ROUTER','VPN','WI-FI','BROWSER', 'IP ADDRESS', 'HYPERLINK','HOTSPOT','SEARCH ENGINE', 'URL', 'WORLD WIDE WEB'] 


ran_num = randrange(0, (len(InternetScrambleWords)))
scramble_rand_word = InternetScrambleWords[ran_num]
points = 0
...
#CODE FOR ALLOWING NEXT BUTTON TO CHANGE WHEN CLICKED!
    def next():
        global ran_num
        ran_num = randrange(0, (len(InternetScrambleWords)))
        word.configure(text=InternetScrambleWords[ran_num])
...
#CODE FOR YOUR RESULT MESSAGEBOX!
    def result():
        ...
        if user_word == InternetCorrectWords[ran_num]:
            points += 10
...
#CODE TO REVEAL CORRECT ANSWER, IF YOU HAVE THE POINTS!
    def reveal_answer():
        ...
        if points > 9:
            points -= 10
            score.configure(text="Score Points ➠  " + str(points))  
            answertext.configure(text=InternetCorrectWords[ran_num])
        else:
...
#CREATING WINDOW FOR HARDWARE (SIZE, IMAGE, LABEL, BUTTON)!
    ...
    window.title("Easy Mode: Internet Word Scramble!")
...
#LABELS & BUTTONS
    ...
    title= Label(text='Internet',pady=10,bg="#D3D3D3",fg="#3cb371",font="Titillium  25 bold")
    title.pack(anchor="n")
    title.place(x=280, y=0)

    scorereminder= Label(text='Can you guess the Internet scrambled words? \n Remember that you can use your score points to find out the answers!',pady=10,bg="#D3D3D3",fg="black",font="Courier 10 ")
